...
user_ret_engage_analysis.write('\n')
user_ret_engage_analysis.markdown('<div style="text-align: justify; font-size: 14px">2. Additionally, the data suggested <i><b>different age groups as well as smartphone users generally prefer a per-book based subscription.</i></b> This has also been supported by market research by Polaris which stated that the segment dedicated to one-time downloads is anticipated to dominate the market, boasting the highest share. <i><b>This is because it empowers users to procure individual audiobooks without obligating them to subscribe to a service. The only exception to this being the ages 25-30 who also show a strong inclination for monthly subscription.</i></b></div>',unsafe_allow_html=True)
user_ret_engage_analysis.write('\n')
